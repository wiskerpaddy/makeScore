...
// =====================================================================
    ...
    // 【全体構造の骨組み】
    // / (1) | (2) (?: (3) | (4) ) | (5) | (6) | (7) /g
    // ※「|」はOR条件、「(?: )」はグループ化するだけでキャプチャしない括弧
    // =====================================================================
    //
    // match[1] : ("[^"]*")
    //            テキスト/コード ("Cm7", "Swing" など。一番優先して除外)
    //
    // match[2] : ((?:![^!]+!|\.)*)
    //            装飾記号やスタッカート (!p! や .)
    //
    // match[3] : (\[[^\]]+\][0-9/]*)
    //            和音 ([CEG]2 など) ※match[2]とセットで判定される
    //
    // match[4] : ([\^_\=]*[a-gA-GzZ][',]*[0-9/]*>?)
    //            単音・休符 (^C'2> など) ※match[2]とセットで判定される
    //
    // match[5] : (\|\]|\|)
    //            小節線 (|] や |)
    //
